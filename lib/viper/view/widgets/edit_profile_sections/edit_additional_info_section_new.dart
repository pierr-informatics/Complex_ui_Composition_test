...
class EditAdditionalInfoSection extends StatefulWidget {
  ...
  const EditAdditionalInfoSection({super.key, required this.profile});

  @override
  State<EditAdditionalInfoSection> createState() =>
      _EditAdditionalInfoSectionState();
}
...
class _EditAdditionalInfoSectionState extends State<EditAdditionalInfoSection> {
  ...
  void initState() {
    ...
    _selectedGender = widget.profile.gender.isNotEmpty
        ...
        : 'Male';
    _selectedEmploymentType = widget.profile.employmentType.isNotEmpty
        ? widget.profile.employmentType
        : 'Full-time';
    _selectedContactMethod = widget.profile.preferredContactMethod.isNotEmpty
        ? widget.profile.preferredContactMethod
        : 'Email';
    _availabilityDate = widget.profile.availabilityDate;
    _expectedSalaryController = TextEditingController(
      text: widget.profile.expectedSalary.toString(),
    );
      ...
      children: [
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
            labelText:
                'Expected Salary (\$${widget.profile.expectedSalary.toStringAsFixed(0)})',
          ...
          onChanged: (value) {
            setState(() {
              widget.profile.expectedSalary =
                  double.tryParse(value) ?? widget.profile.expectedSalary;
            });
          },
        ),
        Slider(
              ...
              child: TextFormField(
                ...
                onChanged: (value) {
                  widget.profile.yearsOfExperience =
                      int.tryParse(value) ?? widget.profile.yearsOfExperience;
                },
              ),
            ),
              ...
              children: [
                IconButton(
                  ...
                  onPressed: () {
                    setState(() {
                      widget.profile.yearsOfExperience++;
                      _yearsOfExperienceController.text = widget
                          .profile
                          .yearsOfExperience
                          .toString();
                    });
